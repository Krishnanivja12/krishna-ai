...
export const ProjectCard = memo(function ProjectCard({ project, index, onClick, variants, limitTags = false }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      variants={variants}
      onClick={handleClick}
      className={`group relative flex flex-col gap-5 rounded-md border glass-premium-bg px-4 py-6 md:p-4 md:py-6 lg:p-8 cursor-pointer transition-all duration-500 ease-out lg:hover:border-primary/40 lg:hover:shadow-xl lg:hover:shadow-primary/5 lg:hover:-translate-y-1 ${isActive ? "border-primary/40" : "border-border"}`}
    >
