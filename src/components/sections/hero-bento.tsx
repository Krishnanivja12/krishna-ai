...
import { useAutoHighlight, useIsMobile, useReducedMotion } from "@/hooks/use-mobile-view-effect"
...
export function HeroBento({ index }: HeroBentoProps) {
  ...
  const ctaButtonClassName = `group inline-flex items-center gap-2 rounded-sm bg-primary px-4 py-2.5 font-mono text-xs font-medium text-primary-foreground transition-all hover:gap-3 sm:px-6 sm:py-3 sm:text-sm sm:gap-3 ${isNameActive ? "gap-4" : ""} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background`

  useEffect(() => {
    if (isMobile) return
    if (hasInteracted) return

    const expandTimer = setTimeout(() => setHasInteracted(true), 1200)
  ...
  const shouldExpand = isHovered || isNameActive || hasInteracted
  ...
  return (
    <section id="home" className="mx-auto max-w-7xl px-4 pt-24 pb-16 md:pt-36 lg:pt-40 lg:pb-24 gpu-layer-paint" aria-labelledby="hero-heading">
      ...
      >
        ...
        <GlowCard as={motion.div} variants={cinematicReveal} className="sm:col-span-2 md:col-span-2 flex flex-col justify-between rounded-md border border-border bg-card/40 backdrop-blur-md p-6 md:p-8 lg:p-10 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            >
              ...
              <motion.span
                ref={nameRef}
                className="text-primary inline-flex cursor-pointer select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-sm"
                tabIndex={0}
                role="button"
              ...
              >
                <motion.span
                  initial={reducedMotion ? { opacity: 1 } : { opacity: 0, filter: "blur(10px)" }}
          ...
          <motion.div
            ...
            transition={{ delay: 0.8, duration: reducedMotion ? 0.01 : 0.5, ease: easePremium }}
            className="mt-6 flex flex-wrap gap-2"
          >
            {expertiseAreas.map((area, i) => (
              <span
                key={area}
                className="inline-flex items-center rounded-sm border border-border/40 bg-card/30 px-2.5 py-1 font-mono text-[10px] text-muted-foreground transition-all duration-300 hover:border-primary/30 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                tabIndex={0}
                role="button"
              >
                <span
          ...
          <motion.div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <MagneticButton>
              <a
                ...
                className="group inline-flex items-center gap-2 rounded-sm border border-border bg-card/40 px-4 py-2.5 font-mono text-xs font-medium text-muted-foreground transition-all hover:border-primary hover:text-foreground hover:gap-3 sm:px-6 sm:py-3 sm:text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
        ...
        {/* Card 2 - Active Shell (Right, spans 2 rows) */}
        <GlowCard
          as={motion.div}
